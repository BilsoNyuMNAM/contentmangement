...
async function syncFromNotion() {
    ...
    let data: Chapter[] = [];
    response.results.forEach((chapterObject)=>{
        ...
        //@ts-ignore
        chapter.Description = chapterObject.properties["description"]?.rich_text?.length > 0 ? chapterObject.properties["description"].rich_text[0].text.content : null;
        //@ts-ignore
        data.push(chapter);
    });
    try{
